...
// 24 SPOOLES 2.2 -- Solving Linear Systems April 29, 2002 A allInOne.c - A Serial LU Driver Program 
...
int main( int argc, char *argv[] ) { 
  ...
  (10) output the solution into binary file
  ...
  char *matrixFileName, *rhsFileName , *mtrXFileName; 
  DenseMtx *mtxY, *mtxX; 
  Chv *rootchv ; 
  ...
  double cpus[10] ; 
  ETree *frontETree ; 
  FILE *inputFile, *msgFile, *mtrXFile ; 
  Graph *graph ; 
  int error, ient, irow, jcol, jrhs, jrow, msglvl, ncol,
    nedges, nent, neqns, nrhs, nrow, pivotingflag, seed, symmetryflag, type ; 
  int *newToOld, *oldToNew ; int stats[20] ; 
  ...
  /*
    April 29, 2002 SPOOLES 2.2 -- Solving Linear Systems 25
  ...
  if ( argc != 10 ) {
    ...
    fprintf(stdout, "\n"
	    ...
	    "\n matrixFileName rhsFileName seed mtrXFileName"
	    ...
	    "\n mtrXFileName -- vector X - solution in binary form" 
	    "\n", argv[0]) ; 
    return(0) ; 
  }
  ...
  /*--------------------------------------------------------------------*/ 
  /*
  //26 SPOOLES 2.2 -- Solving Linear Systems April 29, 2002
  -------------------------------------------- 
  ...
  */ 
  ...
  mtxY = DenseMtx_new() ; 
  DenseMtx_init(mtxY, type, 0, 0, neqns, nrhs, 1, 1);//neqns) ; 
  DenseMtx_zero(mtxY) ; 
  if ( type == SPOOLES_REAL ) {
    double value ; 
    ...
    //April 29, 2002 SPOOLES 2.2 -- Solving Linear Systems 27
  ...
  if ( msglvl > 2 ) {
    ...
    DenseMtx_writeForHumanEye(mtxY, msgFile) ; fflush(msgFile) ; 
  } 
  /*--------------------------------------------------------------------*/ 
  ...
  */ 
  ...
  ChvManager_free(chvmanager) ; if ( msglvl > 2 ) {
  InpMtx_free(mtxA) ; 
  ...
  */ 
  ...
  mtrXFile = fopen(mtrXFileName, "w") ; 
  //DenseMtx_writeToFormattedFile(mtxX,mtrXFile);
  //DenseMtx_writeForHumanEye(mtxX, mtrXFile) ; fflush(mtrXFile) ;
  for(i=0;i<neqns;i++){
    DenseMtx_realEntry (mtxX, i, 0, &v ) ;
    fprintf(mtrXFile,"%le \n",v);
  }
  fclose(mtrXFile); 
  printf("Done!\n");
